...
|---|---|---|
| **Private social event kind** | `PrivateSocialEventPlan.md` | `open` — highest-value item here; **plan doc written 2026-08-12**. Today a private dinner can only be modelled as a GATHERING, which renders in full to anonymous viewers on `/calendar`. **Modeling decided 2026-08-12:** own entry kind, own slice (not a `private` flag on gathering) — so no public gathering projector can leak it. Anonymous view decided 2026-08-07: "Busy" + time range with zone + city/country (`Busy / 7pm–10pm EDT / Toronto, Canada`). Slice = new `EntryKind`, command/event/context, calendar projector, a redacting branch in `CalendarEntryRedactor`, plan form + controller + nav card, and both tiers of redaction test. Change/planned-list/itinerary deferred to follow-ups. Two rendering details open (zone label on redacted time; owner-view format). CLAUDE.md calls the underlying gap a known leak. No live `/calendar` leak exists today (you avoid it by not modelling private events as gatherings). |
