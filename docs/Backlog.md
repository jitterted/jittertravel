...
| Eager-migrate legacy bare-scalar events | `open` — owning doc `LegacyEventEagerMigrationPlan.md` (planned 2026-08-16, nothing built). Rewrite legacy `event_log` payloads to `{utc, zone}` **once** so read-time zone resolution — and per-error resolver hacks like the `Antwerp`/"Brussels" entry — leave the boot-replay path. Detect via "does `EventPayloadUpcaster.upcast` change the JSON?"; validate-then-apply in one transaction; OWNER-only admin action, read-only-guarded; no rebuild needed (fold-equivalent). Retirements it unlocks (upcaster legacy branches, FQCN mapping, resolver hacks) are gated on old backups leaving rotation. Prompted by the 2026-08-16 read-only incident. |
| Boot-replay preflight (pre-deploy) | `open` — no plan doc yet. Replay every stored event through the real read path (upcast → `classFor` → bind, incl. zone resolution) against a prod copy **before** deploying, failing on any bad row. What caught the Morocco/Antwerp failures on 2026-08-16; `/admin/zone-audit` is not a substitute (runtime-only, went stale). Certifies each retirement the eager migration unlocks. |
