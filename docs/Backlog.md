...
| **Calendar subscription feed (cancel-deadline reminders)** | `CalendarSubscriptionFeedPlan.md` | `planned` (2026-08-17) — nothing built. A private, **token-gated** iCalendar (`.ics`) feed Ted subscribes to on iPhone/iPad (built-in Calendar, no app); each hotel booking with a `cancelBy` becomes a VEVENT with **two `VALARM`s — 24h and 4h before** the deadline (the 4h alarm also covers a booking made <24h out) — which **iOS fires locally**, so it works offline/roaming, unlike SMS. Chosen over SMS/email/push because the feed is a **pure projection**: no server scheduler, no 3rd-party provider, and **no exactly-once/replay hazard** (nothing is "sent"). Reuses `BookedHotelView.cancelBy` (no new projector). **Security is the crux:** the feed is **unredacted OWNER data**, so the URL token is the only credential — configured secret (`CALENDAR_FEED_TOKEN`, absent ⇒ 404), constant-time compare, wrong/missing ⇒ 404 empty body, `/calendar/feed/**` matcher + `AuthorizationMatrixTest` row + a dedicated `CalendarFeedSecurityTest`; the subscribe link lives on an OWNER-only `admin-home` card. **Confirmed decisions (2026-08-17):** two alarms (24h + 4h, multiple VALARMs per VEVENT); short timed event at the deadline instant; single shared env token; and a **probe endpoint** `/calendar/feed/{token}/probe.ics` (a synthetic event ~10 min out with a ~5-min alarm, imported one-off) so the on-device alarm path is testable in minutes with **no hotel booking**. **Phase 1 = cancel deadlines only**; the assembler returns a `List<ICalEvent>` so a **full travel calendar** (flights/trains/hotels/gatherings/conferences) can be layered on later — no `ICalEventSource` interface until the 2nd contributor (no abstraction before 2nd user). Open decisions in the doc: fixed 24h vs configurable lead, timed-at-deadline vs all-day event, past-alarm behavior, where to surface the subscribe link, shared-env vs stored-rotatable token. |
